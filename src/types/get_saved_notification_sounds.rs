...
impl GetSavedNotificationSounds {
    ...
    pub fn builder() -> GetSavedNotificationSoundsBuilder {
        let mut inner = GetSavedNotificationSounds::default();
        inner.extra = Some(Uuid::new_v4().to_string());

        inner.td_type = "getSavedNotificationSounds".to_string();

        GetSavedNotificationSoundsBuilder { inner }
    }
}

#[doc(hidden)]
pub struct GetSavedNotificationSoundsBuilder {
    inner: GetSavedNotificationSounds,
}

#[deprecated]
pub type RTDGetSavedNotificationSoundsBuilder = GetSavedNotificationSoundsBuilder;

impl GetSavedNotificationSoundsBuilder {
...
impl AsRef<GetSavedNotificationSounds> for GetSavedNotificationSoundsBuilder {
    fn as_ref(&self) -> &GetSavedNotificationSounds {
        &self.inner
    }
}
